...
**Highlights**
- Shared utilities live in `src/eelgrass_cp` and are reused by the main scripts.
- Scripts are grouped in `scripts/` by domain (conformal, spatial, sensitivity, visualization, inference).
- Outputs are written to user-specified directories so results remain reproducible without changing code structure.
...
- `docs` - Data/paths guidance and notes.

**Quickstart**
1. Create an environment with Python 3.9+.
2. Install dependencies: `pip install -r requirements.txt`
3. Optional (for local imports): `pip install -e .`
...
**Core Workflows (Suggested Order)**
